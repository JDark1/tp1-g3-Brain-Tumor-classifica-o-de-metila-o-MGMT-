...
**Leitura dos resultados — dois níveis de seleção.** Dentro de cada uma das 12
combinações, os hiperparâmetros são escolhidos **estritamente pela validação cruzada
interna** (`GridSearchCV`, 3 dobras, só no treino de cada dobra externa) — nunca
olhando o teste. Já a escolha de **qual das 12 combinações é "a melhor"** foi feita
olhando o `auc_mean` calculado nas próprias dobras de teste externas — isso é seleção
de modelo pelo teste (§10 do TP1, erro nº 4), e por isso o AUC 0,738 do GLCM + LogReg
é otimista. A Seção 8 do notebook 03 corrige exatamente esse segundo nível: em cada
dobra, o par é escolhido só pelo `score_interno` (nota da CV interna no treino), sem
olhar o teste daquela dobra: AUC 0,542 ± 0,138 (agrupado 0,510) — a estimativa sem
viés de seleção. O IC do AUC agrupado do melhor par (0,624) inclui 0,5, e ao
restringir aos exames axiais o sinal desaparece: os 8 pacientes com T1wCE não axial
(3 coronais, 5 sagitais) são todos metilados na amostra, e o modelo tende a
classificá-los como metilados — o **plano de aquisição é um confundidor** (ver
`formulacao_problema.md`). O artigo deve reportar os três números lado a lado (melhor
par, seleção aninhada, só axiais), não só o mais alto. Resultado coerente com Kim et
al. (2022) e Doniselli et al. (2024).
...
- [x] Marco 2 corrigido reexecutado: sem caminho pessoal, com reaproveitamento explícito da
      partição já congelada (não recalculada) e CSVs de resultado regenerados com T1wCE

**Antes da entrega:** (i) conferir se os CSVs regenerados do Marco 2 reproduzem os números já
citados no artigo/`formulacao_problema.md` (AUC do GLCM+LogReg em T1wCE ≈ 0,751 ± 0,114); se
divergirem, atualizar o texto do artigo com os números atuais, não o contrário;
(ii) testar a execução em ambiente limpo, com `USE_CACHE = False` no Marco 3, seguindo apenas
este README.
